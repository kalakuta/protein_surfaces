...
filename = sys.argv[1]
domain = filename.split('/')[-1][:-4]		# strip '.dms' from filename to give pdb domain
txt = open(filename,"r")
surface = {}
direction = {}
type = {}
...
# add in arbitrary rotation for testing scoring measures
# multiple of 60
add_rotation = 60 * numpy.random.randint(1,4)
# random rotation in range (0, 360)
# add_rotation = 360 * numpy.random.rand()
theta = add_rotation * numpy.pi / 180
add_r = rotation_matrix([0, 0, 1], theta)
for point in connected_points:
	coords = [geo_coords[point][0], geo_coords[point][1], 0]
	coords = numpy.dot(add_r, coords)
	geo_coords[point] = [coords[0], coords [1]]
...
for point in connected_points:
	a = orientation[point]
	weighted_orientation[point] = density[point] * a / numpy.sqrt(numpy.dot(a,a))
...
for point in connected_points:
	...
	if d == 0:
		...
		curvatures[point] = numpy.linalg.norm(n1-n0) * step(numpy.linalg.norm(n1 + connected_points[point] - n0) - d) / d
...
file = open('./runs/%s_%s_%s.map' % (domain, centre_point, add_rotation), 'w+')
file.write(domain + '\t')
