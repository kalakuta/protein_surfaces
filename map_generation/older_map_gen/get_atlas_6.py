...
while len(points_list) > 0:
	...
	for point in surface:
		...
		if numpy.linalg.norm(a-b) < 8:
			if point in points_list:
				points_list.remove(point)

	# reduce data set to points within 17 angstroms of centre point
	# map will have 6 rings of cells (including centre) of diameter 3A
	# 
	close_points = {}
	for point in surface:
		a = numpy.array(surface[point])
	...
	# creates a map with 6 concentric hexagonal rings
	...
	file = open('./runs/atlas/%s_%s.map' % (domain,centre_point), 'w+')
	file.write(domain + '\t')
